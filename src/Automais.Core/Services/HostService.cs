...
public class HostService : IHostService
{
    ...
    private async Task<string> BuildSetupScriptAsync(Host host, VpnPeer peer, VpnNetwork vpnNetwork, string publicApiBaseUrl, CancellationToken cancellationToken)
    {
        ...
        sb.AppendLine("  # Pacotes em estado quebrado (ex: linux-image-* no Raspberry Pi) travam qualquer apt install.");
        sb.AppendLine("  # --force-remove-reinstreq pula os scripts pós-remoção que costumam falhar (zz-flash-kernel).");
        sb.AppendLine("  broken=$(dpkg -l 2>/dev/null | awk '/^.[^nci ]/{print $2}')");
        sb.AppendLine("  if [ -n \"$broken\" ]; then");
        sb.AppendLine("    log '[4/6] Pacotes em estado quebrado detectados — forçando remoção para destravar o apt:'");
