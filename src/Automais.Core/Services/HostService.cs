...
public class HostService : IHostService
{
    ...
    private async Task<string> BuildSetupScriptAsync(Host host, VpnPeer peer, VpnNetwork vpnNetwork, string publicApiBaseUrl, CancellationToken cancellationToken)
    {
        ...
        sb.AppendLine("else");
        sb.AppendLine("  log '[4/6] WireGuard ausente — fluxo automático completo (apt) para Ubuntu / Raspberry Pi...'");
        sb.AppendLine("  export DEBIAN_FRONTEND=noninteractive");
        sb.AppendLine("  # Raspberry Pi + Ubuntu: postrm linux-image-* falha em zz-flash-kernel sem este export.");
        sb.AppendLine("  if [ -r /proc/device-tree/model ] && grep -qi raspberry /proc/device-tree/model 2>/dev/null; then");
        sb.AppendLine("    export SKIP_FLASH_KERNEL=1");
        sb.AppendLine("    log '[4/6] Raspberry Pi: SKIP_FLASH_KERNEL=1'");
        sb.AppendLine("  fi");
        sb.AppendLine("  log '[4/6] apt-get update...'");
        sb.AppendLine("  apt-get update");
        sb.AppendLine("  log '[4/6] Corrigindo pacotes pendentes (apt -f / dpkg --configure -a)...'");
        sb.AppendLine("  apt-get install -f -y || true");
        sb.AppendLine("  dpkg --configure -a || true");
