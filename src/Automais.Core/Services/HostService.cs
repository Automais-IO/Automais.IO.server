...
public class HostService : IHostService
{
    ...
    private async Task<string> BuildSetupScriptAsync(Host host, VpnPeer peer, VpnNetwork vpnNetwork, string publicApiBaseUrl, CancellationToken cancellationToken)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("  # zz-flash-kernel (Raspberry Pi / Ubuntu ARM): o postrm de linux-image-* chama");
        sb.AppendLine("  # run-parts /etc/kernel/postrm.d/ que executa zz-flash-kernel; se /lib/firmware/<ver>");
        sb.AppendLine("  # não existir, o find falha e trava qualquer apt/dpkg. Solução: tirar execução");
        sb.AppendLine("  # temporariamente — run-parts ignora arquivos não-executáveis.");
        sb.AppendLine("  ZFK=/etc/kernel/postrm.d/zz-flash-kernel");
        sb.AppendLine("  ZFK_DISABLED=false");
        sb.AppendLine("  if [ -x \"$ZFK\" ]; then");
        sb.AppendLine("    chmod -x \"$ZFK\"");
        sb.AppendLine("    ZFK_DISABLED=true");
        sb.AppendLine("    log '[4/6] zz-flash-kernel temporariamente desativado (Raspberry Pi fix).'");
        sb.AppendLine("  fi");
        sb.AppendLine();
        sb.AppendLine("  # Limpar pacotes em estado quebrado antes de instalar");
        sb.AppendLine("  broken=$(dpkg -l 2>/dev/null | awk '/^.[^nci ]/{print $2}')");
        sb.AppendLine("  if [ -n \"$broken\" ]; then");
        sb.AppendLine("    log '[4/6] Pacotes em estado quebrado — forçando remoção:'");
        sb.AppendLine("    for pkg in $broken; do");
        sb.AppendLine("      log \"[4/6]   -> $pkg\"");
        sb.AppendLine("      dpkg --remove --force-remove-reinstreq \"$pkg\" 2>/dev/null || true");
        sb.AppendLine("    done");
        sb.AppendLine("    apt-get install -f -y 2>/dev/null || true");
        sb.AppendLine("  fi");
        sb.AppendLine();
        sb.AppendLine("  log '[4/6] apt-get update...'");
        sb.AppendLine("  apt-get update");
        sb.AppendLine("  log '[4/6] apt-get install wireguard...'");
        sb.AppendLine("  apt-get install -y wireguard");
        sb.AppendLine("  log '[4/6] WireGuard instalado.'");
        sb.AppendLine();
        sb.AppendLine("  # Restaurar zz-flash-kernel");
        sb.AppendLine("  if [ \"$ZFK_DISABLED\" = true ] && [ -f \"$ZFK\" ]; then");
        sb.AppendLine("    chmod +x \"$ZFK\"");
        sb.AppendLine("    log '[4/6] zz-flash-kernel restaurado.'");
        sb.AppendLine("  fi");
        sb.AppendLine("fi");
        sb.AppendLine();
